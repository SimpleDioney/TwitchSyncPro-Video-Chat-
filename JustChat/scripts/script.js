document.addEventListener("DOMContentLoaded", function () {
      ...
      client.on("connected", (addr, port) => {
        ...
        if (currentChannel) {
          client
            ...
            .catch((error) => {
              console.error(
                `Error joining channel ${currentChannel}:`,
                error
              );
    ...
    function connectToChannel(channel) {
      if (!client) {
        ...
        // Add the message listener again
        client.on("message", (channel, tags, message, self) => {
          if (self) return;
      ...
      client
        ...
        .then(() => {
          ...
          statusElement.textContent = `Connected to ${channel}'s chat${
            isAuthenticated ? ` as ${client.getUsername()}` : ""
          }`;
          statusElement.style.backgroundColor = "var(--success-color)";
        })
        .catch((error) => {
    ...
    sendButton.addEventListener("click", () => {
      ...
      if (message && isAuthenticated && currentChannel) {
        ...
        alert(
          "Please enter a message and make sure you're connected to a channel."
        );
      }
    });
